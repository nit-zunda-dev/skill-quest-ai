...
const App: React.FC = () => {
  ...
  if (!isAuthenticated) {
    ...
    return (
      <PageMeta {...getRouteMeta(PATH_LANDING)}>
        <div className="app-root min-h-screen flex flex-col bg-background text-foreground" data-worldview="arcane-terminal">
          <div className="flex-1 min-h-0 flex flex-col min-w-full">
            <LandingPage onStartClick={() => navigate(PATH_LOGIN)} />
          </div>
          <div className="shrink-0 p-4 text-center text-muted-foreground text-xs">
            Skill Quest AI v1.1.0 &bull; Powered by Workers AI
          </div>
        </div>
      </PageMeta>
  ...
  if (!isShowingGenesisResult) {
    ...
    if (genesisOrProfile.kind === 'error') {
      return (
        <div className="app-root min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4" data-worldview="arcane-terminal">
          <p className="text-destructive" role="alert">{genesisOrProfile.message}</p>
        </div>
      );
    }
  ...
  return (
    <div
      ...
      data-worldview={formData.worldviewId ?? 'arcane-terminal'}
    >
