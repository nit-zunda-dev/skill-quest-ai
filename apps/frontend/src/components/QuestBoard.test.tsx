...
describe('QuestBoard', () => {
  ...
  it('タスク追加ボタンを表示する', () => {
    render(<QuestBoard {...defaultProps} />);
    const addButton = screen.getByRole('button', { name: 'クエストを追加' });
  ...
  it('タスク追加ボタンをクリックするとフォームが表示される', () => {
    ...
    if (addButton) {
      ...
      expect(screen.getByPlaceholderText(/クエスト名を入力/)).toBeDefined();
    }
  });
  ...
  it('タスクを追加できる', () => {
    ...
    const input = screen.getByPlaceholderText(/クエスト名を入力/);
  ...
  it('タスクをステータスごとに分類して表示する', () => {
    ...
    expect(screen.getAllByText(/未着手クエスト/).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/進行中クエスト/).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/達成クエスト/).length).toBeGreaterThan(0);
  });

  it('タスクがない場合に空のメッセージを表示する', () => {
    render(<QuestBoard {...defaultProps} tasks={[]} />);

    const emptyMessages = screen.getAllByText(/クエストなし/);
  ...
  it('タスクをクリックするとステータスが更新される', () => {
    ...
    const taskElement = screen.getAllByText('Test Task')[0];
  ...
  it('タスクの削除ボタンをクリックするとonDeleteTaskが呼ばれる', () => {
    ...
    // ホバーで削除ボタンが表示されるので、直接クリックをシミュレート（複数ビューのうち最初を使用）
    const taskElement = screen.getAllByText('Test Task')[0];
    const taskCard = taskElement.closest('div');
    if (taskCard) {
  ...
  it('Escapeキーでフォームを閉じる', () => {
    ...
    expect(screen.queryByPlaceholderText(/クエスト名を入力/)).toBeNull();
  });
  ...
  it('タスクの難易度バッジを表示する', () => {
    ...
    const badges = screen.getAllByText(Difficulty.MEDIUM);
  ...
  it('繰り返しタスクのストリーク数を表示する', () => {
    ...
    const streakTexts = screen.getAllByText(/連続 5/);
    expect(streakTexts.length).toBeGreaterThan(0);
  });

  it('クエスト0件かつonRequestSuggestFromGoalがあるとき、目標からクエストを生成する案内とCTAを表示する（Task 8.1）', () => {
    ...
    const cta = screen.getByRole('button', { name: /目標からクエストを生成/ });
    expect(cta).toBeDefined();
    expect(screen.getByText(/目標に沿った.*クエストをAIが提案/)).toBeDefined();
  ...
  it('クエスト0件でonRequestSuggestFromGoalがないとき、案内CTAは表示しない（従来の3列の空表示）', () => {
    ...
    expect(screen.queryByRole('button', { name: /目標からクエストを生成/ })).toBeNull();
    const emptyMessages = screen.getAllByText(/クエストなし/);
    expect(emptyMessages.length).toBeGreaterThan(0);
  });
});
