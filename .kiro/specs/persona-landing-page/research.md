...
- **Context**: 要件 3.1 で「デジタルネイティブ・ゲーム・サブカルに親和性が高く、コーポレート風ではない」ことが必要。既存は Skill Quest AI のスレート・インディゴ・Cinzel 等。
- **Alternatives Considered**:
  1. 既存のダークテーマ・フォントをそのままランディングにも適用
  2. ランディングだけ別トーン（例: よりポップ）に差別化
- **Selected Approach**: 既存の Skill Quest AI 世界観（スレート/インディゴのダーク、Cinzel・Zen Kaku Gothic、グラデーション）をランディングでも踏襲する。ヒーローと CTA で「ゲーム感覚で自己研鑽」を前面に出すコピーとレイアウトで差別化する。
