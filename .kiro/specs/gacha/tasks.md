...
- [x] 5.1 クエストを完了にする PATCH（完了専用とステータス更新）の成功後に、付与処理を1回呼び出す
  - 認証とクエストのオーナーであることは既存チェックで担保する
  - _Requirements: 3.1, 3.3, 3.4_
- [x] 5.2 ナラティブ生成完了フローの成功後に、付与処理を1回呼び出す
  - 既存の完了更新の後に呼び出し、認証・オーナーは既存で担保する
  - _Requirements: 3.1, 3.3, 3.4_

- [ ] 6. フロントで所持一覧を取得して表示する
- [x] 6.1 所持一覧 API を呼び出すクライアント処理を追加し、獲得アイテムページで一覧を表示する
  - 認証ユーザー本人の所持のみ取得する
...
- [x] 7.1 抽選がドロップ有効なアイテムのみから選ばれること、同一クエストで2回付与しないこと、マスタ空で付与なしとなることを単体テストで検証する
  - _Requirements: 2.1, 2.3, 2.4, 3.3_
- [x] 7.2 PATCH による完了後に所持が1件増えること、ナラティブ完了後も同様に1件増えること、同じクエストで再度完了しても増えないこと、未認証で一覧が 401 となることを統合テストで検証する
  - _Requirements: 3.1, 3.3, 4.1, 4.3_
- [x]* 7.3 ログインからクエスト完了、獲得アイテム画面で新規取得が表示される流れを E2E で検証する（任意・MVP 後でも可）
  - _Requirements: 4.2_
