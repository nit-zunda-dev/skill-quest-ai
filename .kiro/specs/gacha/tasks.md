...
- [ ] 1.2 (P) アイテムマスタ用の型と所持一覧表示用の型を追加し、画像パス規則を文書化する
...
- [ ] 1.3 (P) 所持一覧 API のレスポンス用 Zod スキーマを追加する
  - 確率値やマスタの許容値はコード内コメントで文書化する
  - _Requirements: 5.3_

- [ ] 2. アイテムマスタとユーザー所持用のテーブルを追加し、マイグレーションと初期データ投入を行う
- [ ] 2.1 アイテムマスタ用テーブルとユーザー所持履歴用テーブルを Drizzle スキーマに定義する
