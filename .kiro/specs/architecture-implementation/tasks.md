...
- [x] 7.7 プロンプトサニタイズとセーフティチェックを実装する
...
- [x] 7.8 AI利用制限を Backend で実装する
  - `docs/architecture/06_AI設計.md` の「利用制限ポリシー」に準拠し、**全4エンドポイント**（キャラ生成・ナラティブ・パートナー・チャット）の制限を本タスクで実施する
  - **generate-character**: 認証ユーザーにつき1回限り。D1で「キャラ生成済み」を記録し、2回目以降は429またはスタブ返却
  - **generate-narrative**: アカウントにつき1日1回。D1（またはKV）で日次利用回数をカウントし、超過時は429またはスタブ返却
  - **generate-partner-message**: アカウントにつき1日1回。同上
  - **chat**: アカウントにつき1日N回（例: 10回）。同上
  - AIルートは認証必須とする（未認証は401）。未認証時のIPフォールバックは別タスクでも可
  - サブスク（subscription_tier）による上限解放・緩和は将来拡張として考慮可能。初回実装では「無料」上限のみでも可
...
- [x] 9.3 AI利用残り回数の表示（任意）
  - Backendに`GET /api/ai/usage`（または既存エンドポイントに含める）を用意し、残り回数・制限情報を返す
  - Frontendでその情報を表示し、制限到達時は該当ボタンを無効化またはメッセージ表示する
...
- [x] 10.2 (P) ログイン/サインアップUIコンポーネントを実装する
