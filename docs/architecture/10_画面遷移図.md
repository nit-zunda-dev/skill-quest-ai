...
| **Genesis QUESTIONS** | プロフィール入力（目標・名前等） | `genesisStep === 'QUESTIONS'` → `QuestionStep` |
| **Genesis LOADING** | キャラクター生成中 | `genesisStep === 'LOADING'` → `LoadingStep` |
| **Genesis RESULT** | キャラ結果表示 | `genesisStep === 'RESULT'` → `ResultStep` |
| **Genesis SUGGEST** | クエスト提案（目標からAIがタスク提案） | `genesisStep === 'SUGGEST'` → `SuggestStep` / `SuggestedQuestsModal` |
| **ダッシュボード** | メイン画面（ルート: `/`, `/quests`, `/grimoire`, `/partner`, `/items`） | `genesisOrProfile.kind === 'dashboard'` または `justCompletedProfile` → `AppLayout` + 各 Page |
...
| ダッシュボード | ナビ・ルート | `/` Home, `/quests`, `/grimoire`, `/partner`, `/items`（`AppLayout`） |
| ダッシュボード | クエストタブ | デイリー / 習慣 / To-Do（`QuestBoard` 内 `activeTab`） |
| ダッシュボード | タスク追加フォーム | 「+」で開閉（`QuestBoard` 内 `isAdding`） |
| ダッシュボード | クエスト報告モーダル | タスク完了押下でオープン。コメント入力 → 結果表示の2段階 |
| ダッシュボード | パートナー | `/partner` または ウィジェット（`PartnerWidget` 内 `isOpen`） |
...
| 役割 | ファイル |
|------|----------|
| 認証・Genesis / 認証済みアプリ分岐 | `apps/frontend/src/App.tsx` |
...
| Genesis 各ステップ | `apps/frontend/src/components/GenesisStep.tsx`, `ResultStep.tsx`, `SuggestStep.tsx`, `SuggestedQuestsModal.tsx` |
| 認証済みレイアウト・ルート | `apps/frontend/src/layouts/AppLayout.tsx`, `apps/frontend/src/pages/*.tsx` |
