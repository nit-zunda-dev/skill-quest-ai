...
- **方針**: 単体テストを主体（70-80%）とし、CI では必ず実行する。統合テストは D1 ローカルバインディングまたはスタブで実行し、Workers AI はスタブまたはスキップ条件で扱う。E2E は Playwright で認証フロー等を自動化済み。ローカルは `pnpm test:e2e:local`、詳細は [E2E テストの実行と CI 準備](../setup/07_e2e_test_setup.md) を参照。
...
- **実行タイミング**:
  - ローカル: サーバー起動後に `pnpm test:e2e:local` で実行（未起動時は Playwright の webServer で自動起動可能）
  - CI: 必要に応じてプレビュー環境デプロイ後の E2E ステップをワークフローに追加する
...
- **OWASP ZAP**: DAST（動的セキュリティテスト）。ベースラインスキャンをローカルで実行。詳細は [OWASP ZAP セットアップ手順](../setup/08_owasp_zap_setup.md) を参照
...
- 新しいテスト種別や CI ジョブを追加する際は、本戦略および `.github/workflows/check.yml` と整合させること。
